      ...
      <header className="md:hidden w-full bg-[#09090B] border-b-2 border-[#3F3F46] px-4 py-3 flex items-center justify-between sticky top-0 z-50">
        <div className="flex items-center space-x-3">
          <motion.div 
            ...
            className="h-8 w-8 bg-[#DFE104] text-black flex items-center justify-center font-bold"
          >
      ...
      <AnimatePresence>
        {mobileOpen && (
          <motion.div 
            ...
            className="md:hidden fixed inset-x-0 top-[59px] bg-[#09090B]/95 backdrop-blur-xl border-b-2 border-[#3F3F46] p-4 z-40 space-y-2 shadow-2xl overflow-hidden"
          >
      ...
      <aside className="w-64 bg-[#09090B] border-r-2 border-[#3F3F46] p-6 flex-col justify-between hidden md:flex shrink-0 min-h-screen">
        <div>
          ...
          <div className="flex items-center space-x-3 mb-8 pb-6 border-b-2 border-[#3F3F46]">
            <motion.div 
              whileHover={{ rotate: 10, scale: 1.1 }}
              className="h-10 w-10 bg-[#DFE104] text-black flex items-center justify-center font-bold"
            >
          ...
          <nav className="space-y-2 text-xs font-heading uppercase font-bold tracking-tighter">
            {navItems.map((item) => {
              ...
              return (
                <motion.div key={item.name} whileHover={{ x: 4 }} transition={{ type: 'spring', stiffness: 400 }}>
                  <Link
                    href={item.href}
                    className={`flex items-center space-x-3 px-4 py-3 border-2 transition-all relative ${
                      isActive
                        ? 'bg-[#DFE104] text-black border-[#DFE104] font-extrabold shadow-md'
                        : 'bg-[#27272A]/40 text-[#A1A1AA] border-transparent hover:border-[#3F3F46] hover:bg-[#27272A] hover:text-[#FAFAFA]'
                    }`}
                  >
