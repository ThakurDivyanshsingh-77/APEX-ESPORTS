...
const submitPayment = asyncHandler(async (req, res) => {
  ...
  const { createPersistentStore } = require('../utils/persistentStore');
  const _tournamentStore = createPersistentStore('tournaments', []);
  ...
  // 1. Write to persistent JSON file store
  ...
  // 2. Write to MongoDB if connected and IDs are valid
  if (
    ...
    try {
      ...
      freshPayments.set(payment._id.toString(), fullPayment);
      ...
      console.warn('[Payment DB Save Warning]', dbError.message);
    }
  }
...
const getMyPayments = asyncHandler(async (req, res) => {
  ...
  const { createPersistentStore } = require('../utils/persistentStore');
  const freshPayments = createPersistentStore('payments', []);
  const mockList = Array.from(freshPayments.values()).filter(
    (p) =>
  ...
  if (mongoose.connection.readyState !== 1) {
    return res
      .status(200)
      .json(new ApiResponse(200, mockList, 'Payment history retrieved (Dev Mode)'));
  }
  ...
  try {
    ...
    const dbList = dbPayments.map((p) => (p.toObject ? p.toObject() : p));
    const dbIds = new Set(dbList.map((d) => String(d._id)));
    const mergedMock = mockList.filter((m) => !dbIds.has(String(m._id)));

    return res
      .status(200)
      .json(new ApiResponse(200, [...dbList, ...mergedMock], 'Payment history retrieved successfully'));
  } catch (error) {
    return res
      .status(200)
      .json(new ApiResponse(200, mockList, 'Payment history retrieved (Dev Mode)'));
...
const getPendingPayments = asyncHandler(async (req, res) => {
  const { createPersistentStore } = require('../utils/persistentStore');
  const freshPayments = createPersistentStore('payments', []);
  const mockList = Array.from(freshPayments.values()).filter((p) => p.status === 'PENDING');

  if (mongoose.connection.readyState !== 1) {
    return res
      .status(200)
      .json(new ApiResponse(200, mockList, 'Pending payments retrieved (Dev Mode)'));
  }

  try {
    const dbPayments = await Payment.find({ status: 'PENDING' })
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, finalList, 'Pending payments retrieved successfully'));
  } catch (error) {
    return res
      .status(200)
      .json(new ApiResponse(200, mockList, 'Pending payments retrieved (Dev Mode)'));
...
const approvePayment = asyncHandler(async (req, res) => {
  ...
  if (mockP) {
    ...
    sendUserNotification({
      ...
      link: `/tournaments/${mockP.tournament?._id || mockP.tournament?.id || ''}`,
    });
  }
    ...
    try {
      ...
      if (payment) {
        ...
        sendUserNotification({
          ...
          type: 'PAYMENT_APPROVED',
          link: `/tournaments/${targetTournId}`,
        });
      ...
      console.warn('[Payment Approval DB Warning]', dbErr.message);
    }
  }
...
const rejectPayment = asyncHandler(async (req, res) => {
  ...
  if (mockP) {
    ...
    sendUserNotification({
      ...
      message: `Your payment for ${mockP.tournament?.title || 'Tournament'} was rejected. Reason: ${remarks || 'Invalid UTR/Proof'}`,
  ...
  if (mongoose.connection.readyState === 1 && mongoose.Types.ObjectId.isValid(id)) {
    try {
      ...
      console.warn('[Payment Rejection DB Warning]', dbErr.message);
    }
  }
