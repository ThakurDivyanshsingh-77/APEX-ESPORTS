  ...
  return (
    <div className="min-h-screen bg-[#09090B] text-[#FAFAFA] pb-20 selection:bg-[#DFE104] selection:text-black">
      ...
      <main className="w-full px-4 sm:px-8 md:px-12 py-8">
        ...
        {/* Filter Controls Bar */}
        <div className="bg-[#09090B] border-2 border-[#3F3F46] p-6 mb-10 space-y-4">
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredTournaments.map((t) => (
              <div
                key={t._id}
                className="bg-[#0F1115] border border-white/10 rounded-3xl overflow-hidden flex flex-col justify-between transition-all duration-300 hover:border-[#F7931A]/60 hover:-translate-y-2 shadow-xl hover:shadow-[0_0_30px_-10px_rgba(247,147,26,0.25)] corner-border-accent"
              >
                {/* Card Banner Image */}
                    ...
                    <span
                      className={`px-3.5 py-1 text-[10px] font-mono font-extrabold uppercase rounded-full border-2 backdrop-blur-md shadow-xl ${
                        t.status === 'LIVE'
                          ...
                          : t.status === 'UPCOMING'
                          ? 'bg-[#030304]/95 text-[#F7931A] border-[#F7931A]/80 shadow-[#F7931A]/30'
                ...
                <div className="p-6 flex-1 flex flex-col justify-between">
                  <div>
                    <h3 className="text-2xl font-heading font-bold uppercase tracking-tight text-white mb-4 leading-snug">
                      {t.title}
                    </h3>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-6 text-xs font-mono font-bold uppercase text-[#94A3B8]">
                      <div className="flex items-center space-x-2">
                        <DollarSign className="h-4 w-4 text-[#FFD600] stroke-[2]" />
                        <span>PRIZE: <strong className="text-[#FFD600]">{t.prizePool}</strong></span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Users className="h-4 w-4 text-[#F7931A] stroke-[2]" />
                  ...
                  <div className="pt-4 border-t border-white/10 flex items-center justify-between">
                    ...
                    <Link
                      ...
                      className="kt-btn-primary !py-2.5 !px-5 text-xs"
                    >
                      VIEW MATCH
                    </Link>
                  </div>
